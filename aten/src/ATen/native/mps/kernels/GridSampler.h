...
struct GridSamplerParams {
  ...
  GridSamplerParams(
    ...
    for (unsigned dim = 0; dim < N; dim++) {
      output_sizes[dim] =
          c10::safe_conv<idx_t>(output.size(dim));
      output_strides[dim] =
          c10::safe_conv<idx_t>(output.stride(dim));
      input_sizes[dim] =
          c10::safe_conv<idx_t>(input.size(dim));
      input_strides[dim] =
          c10::safe_conv<idx_t>(input.stride(dim));
      grid_sizes[dim] = c10::safe_conv<idx_t>(grid.size(dim));
      grid_strides[dim] =
          c10::safe_conv<idx_t>(grid.stride(dim));
    }
  }
#endif
};
